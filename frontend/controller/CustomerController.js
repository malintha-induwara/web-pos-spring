...
const loadCustomersFromBackend = async () => {
  try {
    const response = await fetch("http://localhost:8080/backend/api/v1/customer"); 
    if (!response.ok) {
  ...
  removeButton.addEventListener("click", async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/backend/api/v1/customer/${customer.customerId}`,
        {
          method: "DELETE",
        }
      );
...
customerForm.addEventListener("submit", async (event) => {
  ...
  try {
    let url = "http://localhost:8080/backend/api/v1/customer";
    ...
    if (isUpdateMode) {
      url += `/${currentCustomerId}`; // Append customerId as a query parameter for updates
    }
